...
def main(global_config, **settings):
    ...
    config.add_tween('compulife.auth.authenticator')
    config.include('pyramid_mako')
    config.include('pyramid_handlers')
    config.add_view('pyramid.view.append_slash_notfound_view',
                context='pyramid.httpexceptions.HTTPNotFound')

    add_admin_handler(config, db, get_models(compulife, return_dict=True), 'admin.', '/admin', AdminController)
    #add_admin_handler(config, db, get_models(tools_compulife, return_dict=True), 'admin.', '/admin', AdminController)
